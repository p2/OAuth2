	...
	*/
	public func registerClient(client: OAuth2, callback: ((json: OAuth2JSON?, error: ErrorType?) -> Void)) {
		do {
			...
			client.logIfDebug("Registering client at \(req.URL!) with scopes “\(client.scope ?? "(none)")”")
			client.performRequest(req) { data, status, error in
				do {
					...
					if status >= 400 {
						client.logIfDebug("Registration failed with \(status)")
					}
					else {
	...
	public func registrationRequest(client: OAuth2) throws -> NSMutableURLRequest {
		...
		client.logIfDebug("Registration parameters: \(body)")
	...
	public func didRegisterWith(json: OAuth2JSON, client: OAuth2) {
		if let id = json["client_id"] as? String {
			client.clientId = id
			client.logIfDebug("Did register with client-id “\(id)”, params: \(json)")
		}
		else {
			client.logIfDebug("Did register but did not get a client-id. Params: \(json)")
		}
		if let secret = json["client_secret"] as? String {
			client.clientSecret = secret
			if let expires = json["client_secret_expires_at"] as? Double where 0 != expires {
				client.logIfDebug("Client secret will expire on \(NSDate(timeIntervalSince1970: expires))")
			}
		}
