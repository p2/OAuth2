...
public class OAuth2CodeGrant: OAuth2 {
	...
	*/
	override public func handleRedirectURL(redirect: NSURL) {
		logIfDebug("Handling redirect URL \(redirect.description)")
		do {
	...
	*/
	public func exchangeCodeForToken(code: String) {
		do {
			...
			logIfDebug("Exchanging code \(code) for access token at \(post.URL!)")
			...
			performRequest(post) { data, status, error in
				do {
					...
					if status < 400 {
						self.logIfDebug("Did exchange code for access [\(nil != self.clientConfig.accessToken)] and refresh [\(nil != self.clientConfig.refreshToken)] tokens")
